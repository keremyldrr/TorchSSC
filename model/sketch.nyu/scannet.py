...
class TrainPre(object):
    ...
    def __call__(self, img, hha):
        ...
        p_img = img.transpose(2, 0, 1).astype(float)
...
class ScanNet(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(
            self._path, scene_name, "RGB", "rgb" + unformatted + ".png"
        )
        hha_path = os.path.join(self._path, scene_name, "depth/" + formatted + ".png")
        gt_path = os.path.join(
            self._path, scene_name, "Label/label" + unformatted + ".npz"
        )
        label_weight_path = os.path.join(
            self._path, scene_name, "TSDF/tsdf" + unformatted + ".npz"
        )
        mapping_path = os.path.join(
            self._path, scene_name, "Mapping/mapping" + unformatted + ".npz"
        )
        sketch_path = os.path.join(
            self._path, scene_name, "sketch3d/sketch" + unformatted + ".npz.npy"
        )
        instancedir = os.path.join(
            "../../DATA/ScanNet/",
            scene_name,
        ...
        path_to_depth_intr = os.path.join(
            "../../DATA/scannet_frames_25k/", scene_name, "intrinsics_depth.txt"
        )
        poses = sorted(
            os.listdir(
                os.path.join("../../DATA/scannet_frames_25k/", scene_name, "pose/")
            )
        )
        depths = sorted(
            os.listdir(
                os.path.join("../../DATA/scannet_frames_25k/", scene_name, "depth/")
            )
        )
        pose_path = os.path.join(
            "../../DATA/scannet_frames_25k/",
            scene_name,
            "pose/" + poses[int(unformatted)],
        )
        hha_path = os.path.join(
            "../../DATA/scannet_frames_25k/",
            scene_name,
        ...
        output_dict = dict(
            ...
            tsdf=tsdf.astype(np.float),
            sketch_gt=sketch_gt.astype(float),
            fn=str(item_name),
            n=len(self._file_names),
        )
        # print(torch.Tensor(img).type())
        if self.preprocess is not None and extra_dict is not None:
    ...
    def _fetch_data(
        ...
        VOX_SIZE = 0.08
        TRUNCATION = 0.24
        tsdf = np.load(label_weight_path)["arr_0"].astype(
            np.float32
        )  # .reshape(1, 60, 36, 60)
        ...
        if self.only_box and not self.only_frustum:
            ...
            tsdf = tsdf_from_depth(
                grid_shape,
                VOX_SIZE,
                TRUNCATION,
                depth_intrinsic,
                depth_image_box_filtered,
                worldToCam,
                ptrans,
                upshift=0,
            )
        ...
        return (
            img.astype(np.float),
            hha,
            tsdf.reshape(1, 60, 36, 60).astype(np.float),
            label_weight,
            depth_mapping_3d.astype(np.int64),
            gt.astype(np.int64),
            sketch_gt.reshape(1, 60, 36, 60).astype(np.int64),
        )
