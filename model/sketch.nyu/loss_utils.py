...
def compute_loss(label_weight, label, output,criterion,sketch_gt,pred_sketch_raw,pred_sketch,pred_sketch_gsnn,pred_log_var,pred_mean,engine=None,config=None):
    '''
    semantic loss
    '''
...
def process_one_batch(dataloader,model,config,engine):
    ...
    if model.training:
        ...
        output, _, pred_sketch_raw = model(
            img, depth_mapping_3d, tsdf, sketch_gt)
    if output.requires_grad == False:
        ...
        results_dict = {'pred':pred, 'label':label.detach().cpu(), 'label_weight':label_weight.detach().cpu(),
                        'name': None, 'mapping':depth_mapping_3d.detach().cpu()}
    else:
        results_dict = None
    ...
    if model.training:
        loss, loss_semantic, loss_sketch_raw, loss_sketch, loss_sketch_gsnn, KLD = compute_loss(label_weight, label, output,criterion,sketch_gt,pred_sketch_raw,pred_sketch,pred_sketch_gsnn,pred_log_var,pred_mean,engine,config)
        
        # del tsdf,label_weight, label, output,criterion,sketch_gt,pred_sketch_raw,pred_sketch,pred_sketch_gsnn
        return loss.item(), loss_semantic.item(), loss_sketch_raw.item(), loss_sketch.item(), loss_sketch_gsnn.item(), KLD.item(),results_dict
    else:
        # del tsdf,label_weight, label, output,criterion,sketch_gt,pred_sketch_raw,pred_sketch,pred_sketch_gsnn
        return results_dict