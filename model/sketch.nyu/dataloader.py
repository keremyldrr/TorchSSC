...
def get_train_loader(
    ...
    train_dataset = dataset(
        ...
        s3client=s3client,
        only_frustum=only_frustum,
        only_box=only_box,
    )

    if train_sampler_size:
        dataset_size = len(train_dataset)
        dataset_indices = list(range(dataset_size))
        np.random.shuffle(dataset_indices)
        train_sampler = RandomSampler(dataset_indices[:train_sampler_size])
        batch_size = 1
    else:
        train_sampler = None
...
def get_val_loader(
    ...
    if val_sampler_size:
        ...
        dataset_indices = list(range(dataset_size))
        np.random.shuffle(dataset_indices)
        val_sampler = RandomSampler(dataset_indices[:val_sampler_size])
    else:
        val_sampler = None
    if engine is not None:
        if engine.distributed:
