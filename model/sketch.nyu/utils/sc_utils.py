...
def get_mapping(pts, pts2d, grid_shape):
    ...
    mapping[
        pts[inside_grid_mask][:, 0],
        pts[inside_grid_mask][:, 1],
        pts[inside_grid_mask][:, 2],
    ] = pts2d[inside_grid_mask]
...
def get_camera_pose(curr_pose):
    ...
    """
    camera_pose = pd.read_csv(curr_pose, header=None, delimiter=" ").values[:, :-1]
    return camera_pose
...
def get_grid_to_camera(camera_pose, axis_align_matrix):
    ...
    """
    trns = np.matmul(np.linalg.inv(camera_pose), np.linalg.inv(axis_align_matrix))
    return trns


def get_axis_aligned_matrix(
    scene_name, scan_dir="/home/yildirir/workspace/votenet/scannet/scans/"
):
...
#         instancedir, i)).bounding_box.vertices for i in instances[scores > thresh]]
#     return boxes
...
def get_instance_boxes(instancedir, with_classes=False, thresh=0.3):
    ...
    boxes = [
        trimesh.load(os.path.join(instancedir, i)).bounding_box.vertices
        for i in instances[scores > thresh]
    ]
    # DONE: get the classses here
    if with_classes == False:
        return boxes
    else:
        return boxes, classes[scores > thresh], scores[scores > thresh]
...
def get_pcd_from_depth(depth_image, depth_intrinsic):
    ...
    pts3d = np.matmul(np.linalg.inv(depth_intrinsic)[:3, :3], depth_structure.T).T
...
def tsdf_from_depth(
    ...
    image_coords = vec2
    image_coords[0, :] = image_coords[0, :] / image_coords[2, :]  # .astype(int)
    image_coords[1, :] = image_coords[1, :] / image_coords[2, :]  # .astype(int)
    ...
    homogenImagePositions = np.matmul(np.linalg.inv(depth_intrinsic[:3, :3]), homo)
...
def project_rgb_to_depth(
    curr_rgb_img, color_intrinsic, depth_intrinsic, depth_img, colorToDepthExtr
):
    ...
    img_coords_stacked = np.hstack(
        (rgb_image_inds, np.ones([rgb_image_inds.shape[0], 1]))
    ).astype(int)
    img_coords_rgb_cam = np.matmul(color_intrinsic_inv, img_coords_stacked.T)
    img_coords_depth_cam = (
        np.matmul(colorToDepthExtr[:3, :3], img_coords_rgb_cam).T
        + colorToDepthExtr[:-1, 3]
    )
    img_coords_depth_img = np.matmul(depth_intrinsic[:3, :3], img_coords_depth_cam.T)
    image_in_mask = (
        (img_coords_depth_img[0, :] >= 0)
        & (img_coords_depth_img[1, :] >= 0)
        & (img_coords_depth_img[0, :] < dh)
        & (img_coords_depth_img[1, :] < dw)
    )

    depth_inds = img_coords_depth_img.T[image_in_mask][:, :2].astype(int)
    color_inds = rgb_image_inds[image_in_mask]
    new_img[depth_inds[:, 0], depth_inds[:, 1], 0] = curr_rgb_img[
        color_inds[:, 0], color_inds[:, 1], 0
    ]
    new_img[depth_inds[:, 0], depth_inds[:, 1], 1] = curr_rgb_img[
        color_inds[:, 0], color_inds[:, 1], 1
    ]
    new_img[depth_inds[:, 0], depth_inds[:, 1], 2] = curr_rgb_img[
        color_inds[:, 0], color_inds[:, 1], 2
    ]
    return new_img
...
def read_scan_properties(scan_path, scene_name, label_map, axis_align_matrix):
    ...
    Args:
        ...
        axis_align_matrix ([type]): [description]
    """
    scene_mesh = read_mesh_vertices_rgb(
        os.path.join(scan_path, "{}_vh_clean_2.ply".format(scene_name))
    )
    colors = scene_mesh[:, 3:]
    ...
    object_id_to_segs, label_to_segs = read_aggregation(
        os.path.join(scan_path, "{}_vh_clean.aggregation.json".format(scene_name))
    )
    seg_to_verts, num_verts = read_segmentation(
        os.path.join(scan_path, "{}_vh_clean_2.0.010000.segs.json".format(scene_name))
    )
