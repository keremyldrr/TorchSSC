...
class FocalLoss2d(nn.Module):
    ...
    def forward(self, input, target):
        return self.loss(
            (1 - F.softmax(input, 1)) ** 2 * F.log_softmax(input, 1), target
        )
...
class RCELoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # convert to onehot
        label_pred = (
            torch.zeros(b, self.class_num, h, w).cuda().scatter_(1, target_flat, 1)
        )
...
class SigmoidFocalLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        neg_part = pred_sigmoid ** self.gamma * (
            max_val + ((-max_val).exp() + (-pred_sigmoid - max_val).exp()).log()
        )
